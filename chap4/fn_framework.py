...
# 状態s, 行動a, 報酬r, 遷移先n_s, エピソード終了フラグd
...
class FNAgent():
    ...
    # initialize, estimate, updateはパラメータを持った関数の初期化, 予測, 更新(学習)を行う.
    # 実装は継承したクラスに任せる.
...
class Trainer():
    ...
    def train_loop(self, env, agent, episode=200, initial_count=-1,
        ...
        # observe_interval はどれくらいの頻度で画面をframeに格納するか
        # Agentの行動履歴をexperienceに溜める. buffer_sizeより大きくなると前から捨てる.
    ...
    def is_event(self, count, interval):
        """count > 0 でintervalの倍数か否か"""
        return count and count % interval == 0
